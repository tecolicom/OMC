...
通常はリポジトリ直下の `make` を使う（`make help` で一覧）。

```bash
make update            # 全記事から活動記録を更新（本文・写真込み）= fetch + dedupe
make calendar          # カレンダー反映の確認（dry-run、書き込まない）
make calendar-apply    # 実際に反映
make calendar-apply YEAR=2025   # 年を絞る
make test              # テスト
```

`make` が実行している中身（直接実行する場合の参考）:

```bash
cd calendar
python3 bin/cal-omc-archive-fetch --fetched $(date +%F)   # = make fetch
python3 bin/dedupe-chrome-images sources/blog             # = make dedupe
python3 bin/cal-omc --events-dir events                   # = make calendar (dry-run)
python3 bin/cal-omc --events-dir events --apply           # = make calendar-apply
python3 -m pytest tests/ -q                               # = make test
```
