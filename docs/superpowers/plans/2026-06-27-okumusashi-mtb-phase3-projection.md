...
## 後続（対象外）
- city-tecoli への暦体登録（`/ideas/okumusashi-mtb/` 公開）。**【訂正 2026-06-29】** city-tecoli の設計では
  omc は **Blobs 動的エンティティ**として**管理 UI で登録**する（`global-ideas.yaml`=config seed には入れない）。
  このリポジトリから city-tecoli のファイルは編集しない。
- 自動定期実行（cron）。
